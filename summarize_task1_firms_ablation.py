...
BASE_DIR = Path(".")
EXP_DIR = BASE_DIR / "experiments" / "ia_failure" / "ablation" / "firms"
RESULTS_DIR = BASE_DIR / "results"
RAW_OUT = RESULTS_DIR / "ia_failure_firms_ablation_5seeds_raw.csv"
SUMMARY_OUT = RESULTS_DIR / "ia_failure_firms_ablation_5seeds_summary.csv"
SUMMARY_MD_OUT = RESULTS_DIR / "ia_failure_firms_ablation_5seeds_summary.md"
PAPER_TABLE_OUT = RESULTS_DIR / "ia_failure_firms_ablation_auprc_paper_table.csv"
PAPER_TABLE_MD_OUT = RESULTS_DIR / "ia_failure_firms_ablation_auprc_paper_table.md"
...
def read_metrics(path: Path) -> dict:
    ...
    row = {
        ...
        "ablation_name": payload.get("ablation_name", "firms"),
        "input_protocol": payload.get("input_protocol", "firms"),
...
def simple_markdown(df: pd.DataFrame) -> str:
    if df.empty:
        return ""
    text_df = df.fillna("")
    header = "| " + " | ".join(map(str, text_df.columns)) + " |"
...
def main() -> None:
    ...
    if raw.empty:
        summary = pd.DataFrame(columns=["representation", "model_name", "num_seeds_completed"])
    else:
        grouped = raw.groupby(["representation", "model_name", "model_label"], dropna=False)
        ...
        summary = summary.sort_values(["representation", "model_name"])
    summary.to_csv(SUMMARY_OUT, index=False)
    SUMMARY_MD_OUT.write_text(simple_markdown(summary) + "\n", encoding="utf-8")
    ...
    for representation, model_name in keep_order:
        row = {"Selected model": MODEL_LABELS[model_name], "Representation": representation}
        subset = summary[(summary["representation"] == representation) & (summary["model_name"] == model_name)]
        if subset.empty:
            row["FIRMS-only AUPRC"] = ""
        else:
            r = subset.iloc[0]
            row["FIRMS-only AUPRC"] = fmt_percent(r["test_auprc_mean"], r["test_auprc_std"])
        paper_rows.append(row)
    paper = pd.DataFrame(paper_rows)
    paper.to_csv(PAPER_TABLE_OUT, index=False)
    PAPER_TABLE_MD_OUT.write_text(simple_markdown(paper) + "\n", encoding="utf-8")
